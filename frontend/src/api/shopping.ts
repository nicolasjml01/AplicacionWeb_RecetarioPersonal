import type {
  ...
  export async function searchIngredients(query: string): Promise<IngredientDto[]> {
    const q = query.trim();
    if (!q) return [];
  
    const res = await fetch(`${API_BASE}/api/ingredients?search=${encodeURIComponent(q)}`);
  ...
  export async function getShoppingList(userId: number): Promise<ShoppingListItemDto[]> {
  ...
  export async function patchShoppingItem(
    ...
    const res = await fetch(`${API_BASE}/api/users/${userId}/shopping-list/${itemId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
    });
